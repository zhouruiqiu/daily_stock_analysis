...
export type EvaluationCohort = 'preopen_previous_close' | 'opening_0945' | 'intraday_1000';
export const strategyEvaluationApi = {
  ...
  async leaderboard(horizon: string, cohort: EvaluationCohort) { const { data } = await apiClient.get('/screening/evaluation/leaderboard', { params: { horizon, cohort, window: 20 } }); return toCamelCase(data) as { horizon: string; cohort: EvaluationCohort; items: LeaderboardItem[] }; },
