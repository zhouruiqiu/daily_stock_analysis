...
export default function StrategyLeaderboardPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
    ...
    <section className="overflow-x-auto rounded-2xl border border-border bg-card">
      {loading ? <p className="p-6 text-secondary-text">正在加载...</p> : items.length === 0 ? <p className="p-6 text-secondary-text">尚无已到期样本，首批数据将在评测运行后逐步形成。</p> :
      <table className="min-w-full text-sm"><thead className="border-b border-border text-left text-secondary-text"><tr>{['排名','策略','完整天数','样本','平均收益','超额收益','正收益率','跑赢基准','最大不利波动','状态'].map(x=><th key={x} className="px-4 py-3">{x}</th>)}</tr></thead><tbody>{items.map(item=><tr key={item.strategy} className="border-b border-border/60"><td className="px-4 py-3">{item.rank ?? '-'}</td><td className="px-4 py-3 font-medium">{item.strategy}</td><td className="px-4 py-3">{item.completeDays}</td><td className="px-4 py-3">{item.evaluatedCount}</td><td className="px-4 py-3">{pct(item.avgReturnPct)}</td><td className="px-4 py-3">{pct(item.avgExcessReturnPct)}</td><td className="px-4 py-3">{pct(item.positiveRatePct)}</td><td className="px-4 py-3">{pct(item.beatBenchmarkRatePct)}</td><td className="px-4 py-3">{pct(item.maxAdverseExcursionPct)}</td><td className="px-4 py-3">{item.sampleStatus === 'ranked' ? '已排名' : '样本不足'}</td></tr>)}</tbody></table>}
    </section>
  </div>;
}
