...
- 内置策略显式声明的 `factor_weights` 是实际评分权重；`tech_weight` 只保留为旧自定义策略未提供 `factor_weights` 时的兼容 fallback，内置 YAML 不再同时声明两套权重。
- 当前结构化 `value` 因子只使用 PE/PB 相对排名，不包含 ROE、现金流、负债、股息率或盈利稳定性。低估值策略只生成后续研究候选，不能把“低估值”直接解释为“高质量”。
- `强势题材（实验）` 只接受实时快照；Tushare 日线快照会继续降级到下一实时源，而不是冒充盘中量价数据。候选还必须具有板块热度证据、均线多头、可控的 60 日涨幅/回撤/ATR，再按题材热度与匹配度优先排序。该策略仍没有封板时间、开板次数、封单金额或连板梯队数据，因此不宣传为确定性的打板/连板识别。启用该策略前需配置 `INDUSTRY_PROVIDER=akshare` 或提供包含板块热度字段的行业映射文件。
- `超跌反转` 必须同时满足 60 日阶段下跌、RSI 超卖、波动/回撤/ATR 上限和日 K 数据质量门槛；结果表示修复观察候选，不表示已经止跌或应立即买入。
- `screening_runs` 保存的是运行时筛选结果，便于历史查看与数据源诊断；当前尚未形成筛选策略独立的 T+1/T+5/T+10、基准超额和交易成本表现报告，不能用运行历史条数代替策略有效性验证。
- 启用 `STRATEGY_EVALUATION_ENABLED=true` 后，策略按 YAML `evaluation_profile.cohort` 分时评测：`preopen_previous_close` 在交易日 09:25 使用上一交易日冻结快照并以 09:30 开盘价为收益起点，`opening_0945` 与 `intraday_1000` 分别在 09:45、10:00 使用当时选股价为起点。三个批次在后台 worker 执行，不阻塞盘中主时钟；后续 T+1/T+3/T+5 计算个股收益、沪深300超额收益和最大不利波动。Web `/strategy-leaderboard` 分三榜展示；未满 5 个完整实验日时只显示“样本不足”，不输出正式名次。
- 策略 YAML 加载会拒绝重复 mapping key、倒置的 min/max 区间、负数/非有限因子权重以及全零显式权重，避免静默覆盖或无效评分进入运行时。
