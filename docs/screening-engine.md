...
- 启用 `STRATEGY_EVALUATION_ENABLED=true` 后，交易日 09:25 使用上一交易日冻结快照运行全部内置策略，09:30 同日开盘价作为模拟入场价，并在后续 T+1/T+3/T+5 交易日计算个股收益、沪深300超额收益和最大不利波动。Web `/strategy-leaderboard` 展示排行榜；未满 5 个完整实验日时只显示“样本不足”，不输出正式名次。
