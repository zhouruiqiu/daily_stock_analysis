...
只在“持续上涨/持续下跌/恢复中性”状态切换时推送。若至少 2 个核心指数相对昨收
跌幅达到 `INTRADAY_MARKET_DROP_ALERT_PCT`（默认 1.5%），则立即进入持续下跌
状态并推送，不要求盘中连续 3 个采样点严格单调下降。

启用 `INTRADAY_SCREENING_ENABLED=true` 后，系统会在
交易日 09:45 固定执行一次 `dragon_board` 强势题材选股；此外还会在
`INTRADAY_SCREENING_TIMES`（默认 10:00,14:00）动态选股：持续上涨使用
`dragon_board`，持续下跌使用 `low_volatility_quality`，中性震荡使用
`shrink_pullback`，并将最多 `INTRADAY_SCREENING_MAX_RESULTS`（默认 5）只候选股
通过 `alert` 路由推送。09:45 固定任务不受大盘状态映射影响；选股与大盘监控都会
跳过非交易日。

每只股票的速报同时给出确定性综合评分、建议、风险等级和主要加减分原因，不调用
LLM。数据库中存在持仓时，评分仍由技术面 70 分和成本/浮盈亏 30 分组成，但推送
不展示成本、持仓盈亏、账户金额、持仓数量或由成本推导的防守位。此类明细只在登录后的 Web 查看；非持仓自选使用归一化技术评分，
明确标记为“自选观察”，不会使用“继续持有”等持仓措辞。持仓读取失败时整轮自动
降级为技术观察。真实持仓的有效实时价低于前两个有效支撑位中的最近支撑时，会在
原盯盘推送中置顶显示 `🚨【跌破支撑】`、现价、支撑位和跌破幅度；实时行情缺失时
不会使用日 K 收盘价误触发。评分只是规则化风险提示，不构成投资建议，也不能替代
仓位和账户整体风险管理。

大盘状态变化、周期盯盘和动态选股通知生成后即时发送（`bypass_digest=True`），
不再缓冲到三份综合简报，也不提高严重级别来绕过其他降噪规则。采样和推送频率配置
保持不变：大盘默认每 10 分钟采样、状态变化才推送，持仓盯盘默认每 30 分钟。
盯盘消息使用“结论/风险 → 公开现价涨跌 → 主要理由 → 技术与支撑压力”的紧凑排版；
超长消息沿用编号分片，并优先按股票段落拆分，不提供交互式翻页。

启用 `STRATEGY_EVALUATION_ENABLED=true` 后，系统按策略数据时点在交易日 09:25、
09:45、10:00 分三批运行，并通过 `alert` 路由发送每日 Top N 摘要；每周最后一个
交易日（按交易日历判断，不固定等同周五）15:10 完成后验推进后发送 T+5 周榜。
评测与后验任务在后台 worker 执行，通知成功或失败会写入对应评测批次状态。

同时启用 `PORTFOLIO_TRADING_PLAN_ENABLED=true` 与通知聚合后，15:15 会在发送收盘
综合简报前生成一次只读组合交易计划，作为“持仓盯盘”事件合并到同一条企微消息。
交易计划推送仅包含定性风险状态、逐股规则动作及不涉及账户隐私的理由；净资产、
现金、成本、持仓数量和个人盈亏/回撤数值仅在 Web 展示。旧待发摘要如含账户明细，
发送前替换为隐私提示，避免升级后补发旧金额。交易计划明确声明不构成
收益保证或自动交易指令；计划生成失败不会阻断已经缓冲的其他收盘事件。
