...
- [改进] 大盘状态、周期盯盘和动态选股恢复即时通知，保持原有频率与紧急告警机制；盯盘排版精简并按股票段落优先分片。
- [修复] 持仓及收盘交易计划推送隐藏账户金额、数量、成本和个人盈亏，发送旧待发摘要时屏蔽历史账户明细，认证 Web 数据不变。
...
- [新功能] 新增本地工作区到远端 Systemd 服务的白名单发布脚本，支持发布前备份、健康检查失败自动恢复及按 deployment ID 手动回滚。
- [测试] 补充远程发布脚本的 CLI、运行时白名单、远程安全流程和参数转义回归测试。
- [新功能] 新增企业微信应用消息通知渠道（WECHAT_WORK_CORPID / WECHAT_WORK_AGENTID / WECHAT_WORK_SECRET），可同步至个人微信。
- [新功能] 新增盘内盯盘后台任务（INTRADAY_WATCH_*）：开盘时段每 N 分钟对 STOCK_LIST 做轻量技术面分析（不调 LLM）并推送，午休/盘后/节假日自动跳过。
- [改进] 新增 `--intraday-watch-only` 独立运行模式，使盘内自选推送无需同时启动 Web 或每日完整分析调度。
- [新功能] 盘内自选速报新增持仓感知评分、持有建议、风险等级、成本防守位及可解释的加减分原因，非持仓股票保持观察口径。
- [改进] 强势题材实验策略升级为实时快照与题材证据双门槛，并增加趋势、阶段涨幅、回撤和 ATR 约束，降低纯涨幅候选的排序权重。
- [修复] 实时选股策略遇到 Tushare 日线快照时继续尝试后续实时数据源，并阻止不匹配的快照缓存被复用。
- [新功能] 新增 A 股盘中大盘趋势监控与 10:00/14:00 动态策略选股推送。
- [改进] 盘中任务改为对齐墙上时钟，自选盯盘下午从 13:30 开始，避免任务耗时或重启造成时间漂移。
- [新功能] A 股交易日 09:45 固定执行一次强势题材选股，原 10:00/14:00 动态策略选股保持不变。
- [修复] 行业概念补全成功和缓存命中不再误显示为降级提示，仅保留真实失败、跳过或缓存过期信息。
- [新功能] 大盘至少两个核心指数相对昨收跌幅达到阈值时立即告警，并在原盘中持仓推送中置顶高亮实时价跌破最近支撑的股票。
- [新功能] 新增交易日 09:25 全策略盘前评测、T+1/T+3/T+5 后验收益、Web 策略排行榜及企业微信日报/周榜。
- [修复] 修正策略评测交易日窗口少一天导致 T+5 越界并重启盘中服务的问题；后验失败改为隔离记录，排行榜在收益到期前展示“收益评估中”。
- [新功能] 通知聚合：alert 路由的普通盘中推送缓冲为 10:05/11:35/15:15 三份综合简报，支持去重、发送失败重试与服务重启不重发；大盘急跌、跌破支撑等紧急事件仍立即推送。
- [修复] 大盘复盘与 Agent 行情工具获取指数时改用实时数据源（require_realtime=True），不再回退到上一交易日日线，避免收盘后把旧行情当作当日盘面分析。
- [新功能] 组合交易计划引擎：按均衡型风险规则输出每只持仓的持有/观察/减仓/清仓/确认后加仓建议、止损位与仓位上限，8% 回撤状态机峰值持久化，提供 GET /api/v1/portfolio/trading-plan 只读接口；不自动下单。
- [新功能] 策略评测按 09:25 盘前、09:45 开盘、10:00 盘中三个 cohort 分榜运行，盘中策略使用选股时价格作为后验起点，并把评测/后验任务移入后台 worker。
- [新功能] Web 新增 `/portfolio/trading-plan` 只读交易计划页面，并将每日交易计划合并到 15:15 企业微信收盘简报。
- [改进] 策略周榜按交易日历识别本周最后交易日，评测批次持久化通知成功/失败状态。
- [chore] 删除仓库根目录遗留的推送测试文件 `PUSH_TEST.md`。
