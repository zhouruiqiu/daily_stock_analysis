...
class StrategyEvaluationScheduler:
    ...
    def tick(self, now: Optional[datetime]=None) -> dict[str,Any]:
        ...
        config=self.config_provider(); executed=[]
        ...
        if time_text=="15:10" and key not in self._claims:
            self._claims.add(key); result=self.outcome_service.run_due(current.date(),horizons=list(getattr(config,"strategy_evaluation_horizons",[1,3,5]))); executed.append("outcomes")
